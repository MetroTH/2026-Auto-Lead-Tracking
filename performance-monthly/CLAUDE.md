...
## สถานะล่าสุด (อัปเดต 2026-06-29)

### ✅ ฟีเจอร์ใหม่: คอลัมน์ CPO (Cost Per Order) — Q, R
- **Q `CPO-QT`** = `Amount spent (E)` ÷ จำนวน respond_id distinct ใน **Quotation (D)** (= คอลัมน์ M)
- **R `CPO-INV`** = `Amount spent (E)` ÷ จำนวน respond_id distinct ใน **Invoice (E)** (นับใหม่)
- เพิ่ม helper map: `invCountRaw` (นับ respond_id distinct จาก E ต่อ month+campaign),
  `invMonthDistinct` (นับ distinct ระดับเดือน — ใช้แถว All Campaign)
- คำนวณครบทั้งแถว Campaign / Other (เว้นว่างเพราะไม่มี Spend) / All Campaign
- กันหารด้วยศูนย์: ถ้า Spend ว่าง/0 หรือ ตัวหารว่าง/0 → ปล่อยช่องว่าง
- Output ขยายจาก A–P → **A–R**, number format `0.00` ครอบ P–R

## สถานะก่อนหน้า (อัปเดต 2026-06-24)

### ✅ ใช้งานจริงบนชีตแล้ว (Production)
- Copy `Code.gs` ขึ้น Apps Script ของชีต `Auto-Lead-Tracking-Performance` แล้ว
- ทดสอบรันโหมด 1 (Full) + โหมด 2 (Incremental) ผ่าน — ข้อมูลอัปเดตถึงเดือน มิ.ย. 2026 ✅
- ตั้ง Trigger รายวัน 18:00 (Asia/Bangkok) เรียบร้อย — รันอัตโนมัติทุกวัน

### ✅ ฟีเจอร์ใหม่: แถว "All Campaign" (สรุปยอดรวมรายเดือน)
- แถวสุดท้ายของแต่ละเดือน ต่อจาก "Other" — รวมยอดทั้งเดือน (ทุก Campaign + Other)
- **E,F,G,H,I,N,O** = ผลรวม (sum) | **L(MQL)/M(Lead)** = นับ respond_id distinct ระดับทั้งเดือน
- **J(CTR)/K(CPC)** = เว้นว่าง (อัตราส่วน รวมไม่ได้) | **P(ROAS)** = Sales รวม ÷ Spent รวม
- เหตุผล: แถว Other ไม่มี Spent จึงคำนวณ ROAS ไม่ได้ — แถว All Campaign แก้ปัญหานี้ให้เห็น ROAS ภาพรวม
- helper: `buildMonthDistinctMap_` (นับ distinct ระดับเดือน กันนับซ้ำข้าม campaign), `sumBField_`, `sumMapValues_`

### ℹ️ พฤติกรรม Trigger / โหมด 2 (ยืนยันแล้ว — ไม่เกิดข้อมูลซ้ำ)
- เดือนที่จบไปแล้ว → ข้าม ไม่แตะ
- เดือนปัจจุบัน → `removeMonthRows_` ลบแถวเดิมทิ้งก่อน แล้ว append ใหม่ (refresh) → **ไม่ซ้ำ**
- คอลัมน์ C/D ของแถว Other/All Campaign แสดง "วันสุดท้ายของเดือน" เป็น label เสมอ
  (ไม่ใช่ขอบเขตข้อมูลจริง — ตัวเลขเป็นเท่าที่มีจริง ณ วันที่รัน)

## สถานะก่อนหน้า (2026-06-17)
...
## โครงสร้าง Output Sheet "Performance" — คอลัมน์ A–R

| คอลัมน์ | ชื่อ | แหล่งข้อมูล |
|---|---|---|
| A | Timestamp | เดือน-ปี เช่น Jan-2026 |
| B | Campaign name | B (FBCampaignADS_Part) + "Other" + "All Campaign" |
...
| Q | CPO-QT | E / M (Amount Spent ÷ respond_id distinct ใน Quotation D) |
| R | CPO-INV | E / (respond_id distinct ใน Invoice E) (Amount Spent ÷ จำนวนลูกค้าที่มี Invoice) |
...
- CPO-QT / CPO-INV = '' ถ้า Amount Spent = 0/ว่าง หรือ ตัวหาร (respond_id D/E) = 0/ว่าง
  → แถว "Other" จึงเว้นว่างเสมอ (ไม่มี Spend)
